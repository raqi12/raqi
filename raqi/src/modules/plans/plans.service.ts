...
export class PlansService {
  ...
  private buildFilter(filters?: { activityType?: ActivityType }) {
    if (!filters?.activityType) {
      return {};
    }
    return { activityType: filters.activityType };
  }

  findActive(filters?: { activityType?: ActivityType }): Promise<PlanDocument[]> {
    return this.planModel
      .find({ active: true, ...this.buildFilter(filters) })
      .exec();
  }

  findAll(filters?: { activityType?: ActivityType }): Promise<PlanDocument[]> {
    return this.planModel.find(this.buildFilter(filters)).exec();
  }
